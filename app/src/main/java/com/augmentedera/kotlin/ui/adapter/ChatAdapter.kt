package com.augmentedera.kotlin.ui.adapter
...
import com.augmentedera.kotlin.managers.PROPERTY_NOTIFICATION_TYPE
...
import com.augmentedera.kotlin.ui.activity.PROPERTY_FORWARD_USER_NAME
...
import com.augmentedera.kotlin.utils.getColorCircleDrawable
import com.augmentedera.kotlin.utils.getDateAsHeaderId
import com.augmentedera.kotlin.utils.isAttachmentValid
import com.augmentedera.kotlin.utils.qb.PaginationHistoryListener
import com.augmentedera.kotlin.utils.qb.QbUsersHolder
...
class ChatAdapter(
    ...
    override fun onBindViewHolder(holder: NewMessageViewHolder, position: Int) {
        ...
        if (getItemViewType(position) != TYPE_NOTIFICATION_CENTER) {
            messageLongClickListener?.let {
                holder.rootLayout?.setOnLongClickListener(ItemClickListenerFilter(getItemViewType(
                    position), it, holder, position))
            }
        }
    ...
    private fun fillAttachHolder(
        ...
        setItemAttachClickListener(getAttachListenerByType(position),
            holder,
    ...
    private fun displayAttachment(
        ...
        attachment?.let {
            ...
            val video = QBAttachment.VIDEO_TYPE.equals(attachment.type,
                ignoreCase = true) || attachment.type.contains("video")
            val file =
            ...
            when {
                ...
                video -> {
                    ...
                    holder.tvVideoFileSize?.text =
                        android.text.format.Formatter.formatShortFileSize(context,
    ...
    private fun setItemAttachClickListener(
        ...
        listener?.let {
            holder.rootLayout?.setOnClickListener(ItemClickListenerFilter(getItemViewType(position),
                it,
                holder,
                position))
        }
    ...
    private fun getAttachListenerByType(position: Int): AttachClickListener? {
        ...
        if (attachment != null) {
            ...
            } else if (attachment.type == "file" || attachment.type == "" || attachment.type.contains(
                    "file")
            ) {
    ...
    private inner class ImageLoadListener<M, P>(val holder: NewMessageViewHolder) :
        RequestListener<M, P> {
